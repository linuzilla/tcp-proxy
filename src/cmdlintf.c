...
typedef struct cmd_struct_t {
    char	*name;
    bool	is_rmt;
    int		(*func) (struct cmdlintf_t *, const char *);
    int		cmd_type;
    char	*doc;
    int		nargs;
    struct cmd_struct_t	*subcmd;
    struct cmd_struct_t	*next;
} command_t;
...
static int cmd_history (struct cmdlintf_t *cli, const char *cmd) {
    if (cmd != NULL) {
...
static int cmd_clear_history (struct cmdlintf_t *cli, const char *cmd) {
    if (cmd != NULL) {
        cli->print ("%% Invalid input detected: \"%s\"\n", cmd);
    } else {
        clear_history ();
...
static void external_terminate (void) {
    pdptr->terminate = 1;
}
...
static int bplen (void) {
    return bp_index;
}

static char *bpbptr (void) {
    return bp_buffer;
}
...
static int cli_addcmd (const char *cmd, const bool rmt,
    ...
    for (level = 1; level <= wds; level++) {
        ...
        if (level == wds) {
            if (found) {
                fprintf (stderr, "%s: Duplicate Command\n", cmd);
                return 0;
            } else {
                if (doc != NULL) q->doc = strdup (doc);
                q->func  = cmdfunc;
                q->nargs = args;
                return 1;
            }
...
static int cli_start (const bool is_server, void (*on_success) (void)) {
    int			sockfd;
    ...
    if (pdptr->sockname != NULL) {
        if ((sockfd = socket (PF_UNIX, SOCK_STREAM, 0)) < 0) {
            logger->error (__FILE__, __LINE__, "socket: %s", strerror (errno));
            return 0;
        }
        ...
        if (is_server) {
            ...
            int			cli_pid;
            struct stat stateBuffer;

            if (stat (pdptr->sockname, &stateBuffer) == 0) {
                if (S_ISSOCK (stateBuffer.st_mode)) {
                    if (connect (sockfd, (struct sockaddr *) &serv_addr, servlen) == 0) {
                        logger->warning (__FILE__, __LINE__, "Another server is running");
                        return 0;
                    }

                    logger->warning (__FILE__, __LINE__, "%s: exists, try to remove it", pdptr->sockname);
                    if (unlink (pdptr->sockname) != 0) {
                        logger->warning (__FILE__, __LINE__, "%s: %s", pdptr->sockname, strerror (errno));
                    }
                } else {
                    logger->error (__FILE__, __LINE__, "%s: exists", pdptr->sockname);
                    return 0;
                }
            }
            ...
            if (bind (sockfd, (struct sockaddr *) &serv_addr,
                      servlen) < 0) {
                logger->error (__FILE__, __LINE__, "bind (%s): %s",
                               pdptr->sockname,
                               strerror (errno));
                close (sockfd);
                return 0;
...
static int start_server (void (*on_success) (void)) {
    return cli_start (true, on_success);
}

static int start_client (void) {
    return cli_start (false, NULL);
}

static pthread_mutex_t command_thread_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t command_thread_cond = PTHREAD_COND_INITIALIZER;
static volatile int command_thread_success = -1;

static void change_state_and_wakeup (const int state) {
    command_thread_success = state;
    pthread_mutex_lock (&command_thread_mutex);
    pthread_cond_signal (&command_thread_cond);
    pthread_mutex_unlock (&command_thread_mutex);
}

static void command_main_init_success () {
    change_state_and_wakeup (1);
}

static void *command_main (void *args) {
    start_server (command_main_init_success);
    change_state_and_wakeup (0);
    return NULL;
}

static bool start_server_in_thread (pthread_t *command_thread) {
    pthread_create (command_thread, NULL, command_main, NULL);

    pthread_mutex_lock (&command_thread_mutex);
    while (command_thread_success == -1) {
        pthread_cond_wait (&command_thread_cond, &command_thread_mutex);
    }
    pthread_mutex_unlock (&command_thread_mutex);
    return command_thread_success > 0;
}
...
static const char *const context_name (void) {
    const static char *const name = COMMAND_LINE_INTERFACE_DEFAULT_CONTEXT_NAME;
    return name;
}
...
static struct cmdlintf_t	singleton = {
    .context = {
        .header = {
    ...
    .cli = cmdline_interface,
    .print = lprint,
    .terminate = external_terminate,
    .execute = external_execute,
    .set_login_callback = cli_set_login_callback,
    .set_timeout = cli_set_timeout,
    .regcmd = regist_default_command,
    .set_prompt = cli_set_prompt,
    .start_server = start_server,
    .start_client = start_client,
    .start_server_in_thread = start_server_in_thread,
    .socket_name = cli_socket_name,
    .add = cli_addcmd,
};
